...
const THREAT_COLORS = { CRITICAL:'#ff3a5c', HIGH:'#ff8c00', MODERATE:'#f0c040', LOW:'#00ff9d' };

function applyOverride(zone, delta) {
  if (delta === 0) return zone;
  const newSev = Math.min(10, Math.max(0, (zone.severity_level || 0) + delta));
  const threat_label = newSev > 8 ? 'CRITICAL' : newSev > 6 ? 'HIGH' : newSev > 3 ? 'MODERATE' : 'LOW';
  const priority = Math.min(1, newSev / 10);
  return { ...zone, severity_level: newSev, threat_label, priority, _simDelta: delta };
}
...
const css = `
  ...
  *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
  ...
  @keyframes glow-pulse { 0%,100%{box-shadow:0 0 0 0 rgba(255,58,92,0.5)} 50%{box-shadow:0 0 0 6px rgba(255,58,92,0)} }
  ...
  @keyframes sim-flash { 0%,100%{opacity:1} 50%{opacity:0.5} }
  @keyframes blink { 0%,100%{opacity:1} 50%{opacity:0.3} }
  :root {
    --card: rgba(8,16,40,0.6); --border: rgba(0,180,255,0.12); --border-hi: rgba(0,180,255,0.28);
    --font-mono: 'IBM Plex Mono', monospace; --font-display: 'IBM Plex Mono', monospace;
    --red: #ff3a5c; --orange: #ff8c00; --amber: #00d4ff; --sage: #00ff9d; --muted: #2a4a6a; --faint: #1a3a54;
  }
`;
...
export default function App() {
  ...
  // Dispatch system
  const { dispatches, logEntries, sendDispatch, addLog } = useDispatch({
    zones,
    zoneOverrides,
    setZoneOverrides,
    onZoneOverride: (zoneId, delta) => {
      setZoneOverrides(prev => ({ ...prev, [zoneId]: delta }));
    },
  });

  // Effective alerts: merge backend alerts + sim-triggered ones
  const effectiveAlerts = useMemo(() => {
    const alertMap = {};
    (alerts || []).forEach(a => { alertMap[a.zone] = a; });
    zones.forEach(z => {
      const delta = zoneOverrides[z.zone_id];
      if (!delta) return;
      if ((z.threat_label === 'CRITICAL' || z.threat_label === 'HIGH') && z.priority >= 0.45 && (z.confidence || 0) >= 55) {
        if (!alertMap[z.name]) {
          alertMap[z.name] = {
            alert_id: `sim-${z.zone_id}`,
            zone: z.name, hazard: z.hazard_type,
            urgency_level: z.threat_label,
            confidence_level: z.confidence || 70,
            population: z.population_at_risk || 0,
            recommended_action: `[SIM] Escalated to ${z.threat_label} — deploy emergency resources immediately.`,
            resources_dispatched: [], timestamp: new Date().toISOString(),
            threat_color: THREAT_COLORS[z.threat_label],
          };
        } else {
          alertMap[z.name] = { ...alertMap[z.name], urgency_level: z.threat_label, threat_color: THREAT_COLORS[z.threat_label] };
        }
      }
    });
    return Object.values(alertMap);
  }, [alerts, zones, zoneOverrides]);
      ...
      <header style={{height:'52px',flexShrink:0,display:'flex',alignItems:'center',gap:'16px',padding:'0 20px',background:'rgba(3,6,18,0.9)',backdropFilter:'blur(24px)',borderBottom:'1px solid rgba(0,180,255,0.1)',zIndex:10}}>
        <div style={{display:'flex',alignItems:'center',gap:'10px'}}>
          ...
          <div>
            ...
            <div style={{fontFamily:'IBM Plex Mono',fontSize:'8px',color:'#1a3555',letterSpacing:'0.14em'}}>AUTONOMOUS HUMANITARIAN RESPONSE</div>
          </div>
        </div>
        ...
        {[
          ...
          {label:'RESOURCES', value:totalResLeft, color: totalResLeft<20?'#ff3a5c':'#00ff9d'},
        ...
        {simActive && (
          <div style={{display:'flex',alignItems:'center',gap:'8px',background:'rgba(255,140,0,0.1)',border:'1px solid rgba(255,140,0,0.3)',borderRadius:'8px',padding:'4px 12px'}}>
            ...
            <button onClick={() => setZoneOverrides({})} style={{marginLeft:'6px',background:'rgba(255,58,92,0.1)',border:'1px solid rgba(255,58,92,0.3)',borderRadius:'5px',padding:'2px 8px',color:'#ff3a5c',fontSize:'8px',fontFamily:'IBM Plex Mono',cursor:'pointer',letterSpacing:'0.1em'}}>✕ RESET</button>
          </div>
        )}

        <div style={{width:'1px',height:'26px',background:'rgba(0,180,255,0.1)',marginLeft:'auto'}}/>
      ...
      <div style={{flex:1,display:'flex',overflow:'hidden',minHeight:0}}>
        ...
        <aside style={{width:'260px',flexShrink:0,background:'rgba(3,6,18,0.85)',backdropFilter:'blur(18px)',borderRight:'1px solid rgba(0,180,255,0.09)',display:'flex',flexDirection:'column',overflow:'hidden'}}>
          <div style={{fontFamily:'IBM Plex Mono',fontSize:'7.5px',letterSpacing:'0.16em',color:'#1a3a54',padding:'8px 14px 6px',borderBottom:'1px solid rgba(0,180,255,0.07)',display:'flex',alignItems:'center',gap:'7px'}}>
            <span style={{width:'4px',height:'4px',borderRadius:'50%',background:'#38bdf8',display:'inline-block'}}/>
            SITUATION REPORT
          </div>
          ...
          <div style={{flex:1,overflowY:'auto'}}>
            {zones.map(z => {
              ...
              const zoneDispatches = dispatches.filter(d => d.zoneId === z.zone_id && d.status !== 'returned');
              return (
                <div key={z.zone_id} onClick={()=>setSelectedZone(z)} style={{padding:'9px 14px',borderBottom:'1px solid rgba(56,120,200,0.07)',cursor:'pointer',background:sel?'rgba(56,189,248,0.06)':'transparent',borderLeft:sel?`2px solid ${tc}`:'2px solid transparent',transition:'all 0.15s',position:'relative'}}>
                  <div style={{display:'flex',alignItems:'center',gap:'7px',fontSize:'10px',color:'#8eacc8'}}>
                    <span>{HAZARD_ICONS[z.hazard_type]||'◉'}</span>
                    <span style={{overflow:'hidden',textOverflow:'ellipsis',whiteSpace:'nowrap',flex:1}}>{z.name}</span>
                    {zoneDispatches.length > 0 && (
                      <span style={{fontSize:'7px',color:'#f97316',background:'rgba(249,115,22,0.1)',padding:'1px 5px',borderRadius:'3px',border:'1px solid rgba(249,115,22,0.3)',flexShrink:0}}>
                        {zoneDispatches.map(d => VEHICLE_TYPES_ICONS[d.vehicleKey]||'🚗').join('')}
                      </span>
                    )}
                    {delta !== 0 && (
                      <span style={{fontSize:'7px',color:delta>0?'#ff8c00':'#00ff9d',background:delta>0?'rgba(255,140,0,0.1)':'rgba(0,255,157,0.1)',padding:'1px 5px',borderRadius:'3px',border:`1px solid ${delta>0?'rgba(255,140,0,0.3)':'rgba(0,255,157,0.3)'}`,flexShrink:0}}>{delta>0?'+':''}{delta.toFixed(1)}</span>
                    )}
                    <span style={{fontSize:'7px',fontWeight:600,letterSpacing:'0.09em',padding:'1px 5px',borderRadius:'3px',color:tc,background:`${tc}18`,border:`1px solid ${tc}35`,flexShrink:0}}>{z.threat_label}</span>
                  </div>
                  <div style={{display:'flex',gap:'10px',fontSize:'8px',color:'#2a4060',marginTop:'4px'}}>
                    <span>SEV {(z.severity_level||0).toFixed(1)}</span>
                    <span>CONF {z.confidence}%</span>
                    <span>{((z.population_at_risk||0)/1000).toFixed(1)}K</span>
                  </div>
          ...
          <div style={{borderTop:'1px solid rgba(0,180,255,0.08)',padding:'10px 14px',flexShrink:0}}>
            <div style={{fontFamily:'IBM Plex Mono',fontSize:'8px',letterSpacing:'0.16em',color:'#1a3a54',marginBottom:'8px',display:'flex',alignItems:'center',gap:'6px'}}>
              <span style={{width:'4px',height:'4px',borderRadius:'50%',background:'#00d4ff',display:'inline-block'}}/>
              RESOURCE POOL
            </div>
            {Object.entries(resources).map(([k,r]) => {
              ...
              return (
                <div key={k} style={{display:'flex',alignItems:'center',gap:'8px',marginBottom:'6px'}}>
                  <span style={{fontFamily:'IBM Plex Mono',fontSize:'7px',color:'#1e3a58',width:'80px',textTransform:'uppercase',letterSpacing:'0.05em',overflow:'hidden',textOverflow:'ellipsis',whiteSpace:'nowrap'}}>{k.replace(/_/g,' ')}</span>
        ...
        <div style={{flex:1,display:'flex',flexDirection:'column',overflow:'hidden',minWidth:0}}>
          <div style={{height:'40px',flexShrink:0,display:'flex',alignItems:'center',gap:'2px',padding:'0 16px',background:'rgba(3,6,18,0.7)',backdropFilter:'blur(14px)',borderBottom:'1px solid rgba(0,180,255,0.07)'}}>
            {TABS.map(t => {
              ...
              const badge = t.id === 'logs' ? logEntries.length : t.id === 'alerts' ? effectiveAlerts.length : null;
              return (
                <button key={t.id} onClick={()=>setActiveTab(t.id)} style={{display:'flex',alignItems:'center',gap:'5px',padding:'5px 14px',background:isActive?'rgba(0,180,255,0.1)':'transparent',border:'none',borderRadius:'7px',color:isActive?'#00d4ff':'#243a56',fontFamily:'Inter',fontSize:'12px',fontWeight:500,cursor:'pointer',transition:'all 0.15s',boxShadow:isActive?'0 0 0 1px rgba(0,180,255,0.2)':'none',outline:'none',position:'relative'}}>
                  ...
                  {t.label}
                  {badge > 0 && (
                    <span style={{position:'absolute',top:'2px',right:'4px',background:t.id==='alerts'?'#ff3a5c':'rgba(0,180,255,0.3)',borderRadius:'8px',padding:'0 4px',fontSize:'7px',fontFamily:'IBM Plex Mono',color:'white',minWidth:'14px',textAlign:'center',lineHeight:'14px'}}>{badge > 99 ? '99+' : badge}</span>
                  )}
                </button>
              );
            })}
          ...
          <div style={{flex:1,overflow:'auto',padding:'14px',minHeight:0,background:'rgba(3,6,18,0.3)'}}>
            ...
            {activeTab==='overview' && (
              <div style={{display:'grid',gridTemplateColumns:'1fr 360px',gap:'14px',height:'100%',minHeight:0}}>
                <div style={{...glass(),display:'flex',flexDirection:'column',minHeight:0,overflow:'hidden',padding:0}}>
                  <div style={{padding:'10px 16px',borderBottom:'1px solid rgba(0,180,255,0.09)',display:'flex',alignItems:'center',justifyContent:'space-between',flexShrink:0}}>
                    <div style={{display:'flex',alignItems:'center',gap:'7px'}}>
                      ...
                      <span style={{fontFamily:'IBM Plex Mono',fontSize:'9px',color:'#1a4060',letterSpacing:'0.16em'}}>LIVE SITUATION MAP</span>
                    </div>
                    <div style={{display:'flex',alignItems:'center',gap:'8px'}}>
                      {activeDispatches > 0 && (
                        <span style={{fontFamily:'IBM Plex Mono',fontSize:'8px',color:'#f97316',background:'rgba(249,115,22,0.1)',padding:'2px 10px',borderRadius:'5px',border:'1px solid rgba(249,115,22,0.25)',animation:'sim-flash 2s infinite'}}>
                          {activeDispatches} UNIT{activeDispatches>1?'S':''} DEPLOYED
                        </span>
                      )}
                      {simActive && !activeDispatches && (
                        <span style={{fontFamily:'IBM Plex Mono',fontSize:'8px',color:'#ff8c00',background:'rgba(255,140,0,0.1)',padding:'2px 10px',borderRadius:'5px',border:'1px solid rgba(255,140,0,0.2)',animation:'sim-flash 1.5s infinite'}}>⚡ SIMULATION MODE</span>
                      )}
                    </div>
                  </div>
                  <div style={{flex:1,minHeight:0}}>
                    <CrisisMap
                      ...
                      onZoneOverride={(zoneId, delta) => setZoneOverrides(prev => ({ ...prev, [zoneId]: Math.min(5, Math.max(-5, (prev[zoneId]||0)+delta)) }))}
                      onZoneOverrideReset={(zoneId) => setZoneOverrides(prev => { const c={...prev}; delete c[zoneId]; return c; })}
                ...
                <div style={{display:'flex',flexDirection:'column',gap:'12px',overflow:'auto',maxHeight:'100%'}}>
                  <div style={{...glass({padding:'14px'})}}>
                    ...
                    <div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:'8px'}}>
                      {[
                        ...
                        {label:'DISPATCHES', value:activeDispatches, color:activeDispatches>0?'#f97316':'#2a4a6a'},
                      ].map(({label,value,color})=>(
                  ...
                  <div style={{...glass({padding:'14px'}),overflow:'visible',minHeight:0}}>
            ...
            {activeTab==='alerts' && (
              <div style={{...glass({padding:'16px'})}}>
                <div style={{fontFamily:'IBM Plex Mono',fontSize:'8px',letterSpacing:'0.16em',color:'#1a4060',marginBottom:'14px'}}>ACTIVE ALERTS</div>
            ...
            {activeTab==='trends' && (
              <div style={{...glass({padding:'16px'})}}>
                <div style={{fontFamily:'IBM Plex Mono',fontSize:'8px',letterSpacing:'0.16em',color:'#1a4060',marginBottom:'14px'}}>TREND ANALYSIS</div>
            ...
            {activeTab==='strategy' && (
              <div style={{...glass({padding:'16px'})}}>
                <div style={{fontFamily:'IBM Plex Mono',fontSize:'8px',letterSpacing:'0.16em',color:'#1a4060',marginBottom:'14px'}}>RESPONSE STRATEGY</div>
...
// Small helper for sidebar dispatch icons
const VEHICLE_TYPES_ICONS = {
  helicopter:'🚁', ambulance:'🚑', fire_truck:'🚒',
  rescue_team:'🚐', coast_guard:'🚢', military:'🪖', drone:'🛸',
};
