...
const HAZARD_ICONS = {
  ...
  Wildfire:'🔥', 'Disease Outbreak':'☣', Landslide:'⛰', Conflict:'⚡',
};
const THREAT_COLORS = {
  CRITICAL:'#ff3a5c', HIGH:'#ff8c00', MODERATE:'#f0c040', LOW:'#00ff9d',
};

/** Recompute derived fields from severity_level */
...
const css = `
  ...
  *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
  ...
  :root {
    --card: rgba(8,16,40,0.6);
    --border: rgba(0,180,255,0.12);
    --border-hi: rgba(0,180,255,0.28);
    --font-mono: 'IBM Plex Mono', monospace;
    --font-display: 'IBM Plex Mono', monospace;
    --red: #ff3a5c; --orange: #ff8c00; --amber: #00d4ff;
    --sage: #00ff9d; --muted: #2a4a6a; --faint: #1a3a54;
  }
`;
...
const glass = (extra = {}) => ({
  ...
  borderRadius: '14px',
  boxShadow: '0 4px 32px rgba(0,0,0,0.5), inset 0 1px 0 rgba(0,180,255,0.07)',
  ...extra,
});
...
export default function App() {
  ...
  // zoneOverrides: { [zone_id]: deltaSeverity }
  const [zoneOverrides, setZoneOverrides] = useState({});

  const handleStratRegen = useCallback(async () => {
    setActiveTab('strategy');
    setRegenLoading(true);
    try { await regenerateStrategy(); setTimeout(refresh, 3500); }
    catch {}
    finally { setTimeout(() => setRegenLoading(false), 3500); }
  }, [refresh]);
  ...
  const logs          = state?.decision_log ?? state?.logs ?? state?.log ?? state?.decision_logs ?? [];
  const citizenPings  = state?.citizen_pings ?? [];
  const cycle         = state?.cycle         ?? 0;
  const simParams     = state?.simulation_params ?? {};

  // Apply overrides to produce effectiveZones used everywhere
  const zones = useMemo(() =>
    rawZones.map(z => applyOverride(z, zoneOverrides[z.zone_id] || 0)),
    [rawZones, zoneOverrides]
  );

  // Recompute alerts based on effective zones
  const effectiveAlerts = useMemo(() => {
    // Merge original alerts with any newly-critical zones from simulation
    const alertMap = {};
    ...
    zones.forEach(z => {
      ...
      if (!delta) return;
      // If zone became critical/high through simulation and doesn't have an alert yet, synthesize one
  ...
  return (
    ...
    <div style={{position:'relative',zIndex:1,height:'100vh',width:'100vw',display:'flex',flexDirection:'column',overflow:'hidden'}}>
      ...
      <header style={{height:'52px',flexShrink:0,display:'flex',alignItems:'center',gap:'16px',padding:'0 20px',background:'rgba(3,6,18,0.9)',backdropFilter:'blur(24px)',borderBottom:'1px solid rgba(0,180,255,0.1)',zIndex:10}}>
        ...
        {simActive && (
          <div style={{marginLeft:'auto',display:'flex',alignItems:'center',gap:'8px',background:'rgba(255,140,0,0.1)',border:'1px solid rgba(255,140,0,0.3)',borderRadius:'8px',padding:'4px 12px'}}>
            ...
            <button
              onClick={() => setZoneOverrides({})}
              style={{marginLeft:'6px',background:'rgba(255,58,92,0.1)',border:'1px solid rgba(255,58,92,0.3)',borderRadius:'5px',padding:'2px 8px',color:'#ff3a5c',fontSize:'8px',fontFamily:'IBM Plex Mono',cursor:'pointer',letterSpacing:'0.1em'}}
            >
              ✕ RESET
            </button>
          </div>
        ...
        <div style={{width:'1px',height:'26px',background:'rgba(0,180,255,0.1)',marginLeft: simActive ? 0 : 'auto'}}/>
      ...
      <div style={{flex:1,display:'flex',overflow:'hidden',minHeight:0}}>
        ...
        <aside style={{width:'260px',flexShrink:0,background:'rgba(3,6,18,0.85)',backdropFilter:'blur(18px)',borderRight:'1px solid rgba(0,180,255,0.09)',display:'flex',flexDirection:'column',overflow:'hidden'}}>
          ...
          <div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:'1px',background:'rgba(0,180,255,0.05)',flexShrink:0}}>
            {[
              ...
              {l:'ACTIVE ZONES',v:zones.length},
            ].map(({l,v,crit,warn})=>(
              <div key={l} style={{padding:'10px 12px',background:'rgba(12,24,40,0.6)'}}>
                <div style={{fontFamily:'IBM Plex Mono',fontWeight:800,fontSize:'22px',color:crit?'#ff3a5c':warn?'#ff8c00':'#38bdf8',lineHeight:1}}>{v}</div>
              ...
              return (
                <div key={z.zone_id} onClick={()=>setSelectedZone(z)} style={{padding:'9px 14px',borderBottom:'1px solid rgba(56,120,200,0.07)',cursor:'pointer',background:sel?'rgba(56,189,248,0.06)':'transparent',borderLeft:sel?`2px solid ${tc}`:'2px solid transparent',transition:'all 0.15s',position:'relative'}}>
                  <div style={{display:'flex',alignItems:'center',gap:'7px',fontSize:'10px',color:'#8eacc8'}}>
                    ...
                    {delta !== 0 && (
                      <span style={{fontSize:'7px',color:delta>0?'#ff8c00':'#00ff9d',background:delta>0?'rgba(255,140,0,0.1)':'rgba(0,255,157,0.1)',padding:'1px 5px',borderRadius:'3px',border:`1px solid ${delta>0?'rgba(255,140,0,0.3)':'rgba(0,255,157,0.3)'}`,flexShrink:0}}>
        ...
        <div style={{flex:1,display:'flex',flexDirection:'column',overflow:'hidden',minWidth:0}}>
          ...
          <div style={{height:'40px',flexShrink:0,display:'flex',alignItems:'center',gap:'2px',padding:'0 16px',background:'rgba(3,6,18,0.7)',backdropFilter:'blur(14px)',borderBottom:'1px solid rgba(0,180,255,0.07)'}}>
            {TABS.map(t => (
              <button key={t.id} onClick={()=>setActiveTab(t.id)} style={{display:'flex',alignItems:'center',gap:'5px',padding:'5px 14px',background:activeTab===t.id?'rgba(0,180,255,0.1)':'transparent',border:'none',borderRadius:'7px',color:activeTab===t.id?'#00d4ff':'#243a56',fontFamily:'Inter',fontSize:'12px',fontWeight:500,cursor:'pointer',transition:'all 0.15s',boxShadow:activeTab===t.id?'0 0 0 1px rgba(0,180,255,0.2)':'none',outline:'none'}}>
                <span style={{fontSize:'11px',opacity:activeTab===t.id?1:0.4}}>{t.icon}</span>{t.label}
              </button>
            ))}
          </div>
          ...
          <div style={{flex:1,overflow:'auto',padding:'14px',minHeight:0,background:'rgba(3,6,18,0.3)'}}>
              ...
              <div style={{display:'grid',gridTemplateColumns:'1fr 360px',gap:'14px',height:'100%',minHeight:0}}>
                ...
                <div style={{...glass(),display:'flex',flexDirection:'column',minHeight:0,overflow:'hidden',padding:0}}>
                  <div style={{padding:'10px 16px',borderBottom:'1px solid rgba(0,180,255,0.09)',display:'flex',alignItems:'center',justifyContent:'space-between',flexShrink:0}}>
                    ...
                    {simActive && (
                      <span style={{fontFamily:'IBM Plex Mono',fontSize:'8px',color:'#ff8c00',background:'rgba(255,140,0,0.1)',padding:'2px 10px',borderRadius:'5px',border:'1px solid rgba(255,140,0,0.2)',animation:'sim-flash 1.5s infinite'}}>
                        ⚡ SIMULATION MODE
                      </span>
                    )}
                    {selectedZone && !simActive && (
                      <span style={{fontFamily:'IBM Plex Mono',fontSize:'10px',color:'#00d4ff',background:'rgba(0,180,255,0.1)',padding:'2px 10px',borderRadius:'5px',border:'1px solid rgba(0,180,255,0.2)'}}>
                        {selectedZone.name}
                      </span>
                    )}
                  </div>
                    ...
                    <CrisisMap
                      ...
                      onZoneOverride={(zoneId, delta) => {
                        setZoneOverrides(prev => ({
                          ...prev,
                          [zoneId]: Math.min(5, Math.max(-5, (prev[zoneId] || 0) + delta))
                        }));
                      }}
                      onZoneOverrideReset={(zoneId) => {
                        setZoneOverrides(prev => {
                          const copy = { ...prev };
                          delete copy[zoneId];
                          return copy;
                  ...
                  <div style={{...glass({padding:'14px'})}}>
                    ...
                    <div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:'8px'}}>
                      {[
                        ...
                        {label:'PINGS',    value:citizenPings.length,        color:'#a78bfa'},
                      ].map(({label,value,color})=>(
            ...
            {activeTab==='alerts' && (
              <div style={{...glass({padding:'16px'})}}>
                ...
                <AlertsTab alerts={effectiveAlerts}/>
              </div>
            )}
            ...
            {activeTab==='logs' && (
              <div style={{...glass({padding:'16px'})}}>
                <div style={{fontFamily:'IBM Plex Mono',fontSize:'8px',letterSpacing:'0.16em',color:'#1a4060',marginBottom:'14px'}}>DECISION LOG</div>
                <LogsTab logs={logs}/>
              </div>
            )}
